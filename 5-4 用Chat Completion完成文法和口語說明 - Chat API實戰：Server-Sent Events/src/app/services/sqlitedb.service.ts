...
import {
  AILastGrammerAndColloquialModel,
  ChatHistoryModel,
  ChatRoomModel,
} from '../models/sqlitedb.model';
...
export class SqlitedbService {
  ...
  public async getAILastGrammerAndColloquialAsync(): Promise<AILastGrammerAndColloquialModel> {
    const defaultResult: AILastGrammerAndColloquialModel = {
      grammar: false,
      colloquial: false,
    };
    try {
      //只讀取當前選中的聊天室的歷史訊息資料
      const userLastSentenceData = await this.db.query(
        'SELECT CHATHISTORY.grammar, CHATHISTORY.colloquial FROM CHATHISTORY JOIN CHATROOM ON CHATHISTORY.chatRoomId = CHATROOM.chatRoomId WHERE CHATROOM.isSelected = 1 AND CHATHISTORY.role = "assistant" ORDER BY CHATHISTORY.timestamp DESC LIMIT 1'
      );
      return userLastSentenceData.values
        ? {
            grammar:
              userLastSentenceData.values[0].grammar === 1 ? true : false,
            colloquial:
              userLastSentenceData.values[0].colloquial === 1 ? true : false,
          }
        : defaultResult;
    } catch (error) {
      console.error('Error loading user last sentence data:', error);
      return defaultResult;
    }
  }

  public async getUserLastSentenceAsync(): Promise<string> {
    try {
      //只讀取當前選中的聊天室的歷史訊息資料
      const userLastSentenceData = await this.db.query(
        'SELECT CHATHISTORY.content FROM CHATHISTORY JOIN CHATROOM ON CHATHISTORY.chatRoomId = CHATROOM.chatRoomId WHERE CHATROOM.isSelected = 1 AND CHATHISTORY.role = "user" ORDER BY CHATHISTORY.timestamp DESC LIMIT 1'
      );
      return userLastSentenceData.values
        ? userLastSentenceData.values[0].content
        : '';
    } catch (error) {
      console.error('Error loading user last sentence data:', error);
      return '';
